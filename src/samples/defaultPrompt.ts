...
**文字だけ箇条書きのスライドは全体の 30% 以下に抑える** こと。
...
- **プロセスフロー (FLOW)** ×1: 段取り・タイムラインは必ず矢印フローで。
  例: \`## FLOW 4ヶ月の実装ロードマップ\`
       \`- 設計 | 5月末\` / \`- 実装 | 6月末\` / \`- ベータ | 7月中旬\`
- **3カードグリッド (CARDS)** ×1: 並列 3 要素は色分けカードで。
  例: \`## CARDS 提供する 3 つの価値\`
       \`- 即応性 | 1 秒以内の通知\` / \`- 発見性 | 検索精度 +30%\` / \`- 信頼性 | SOC2 準拠\`
- **比較表 (table)** ×1: 比較・優先順位は Markdown テーブルで。
- **引用 (quote)** ×1: 経営層に響く一文を引用ブロックで強調。
...
- \`## FLOW タイトル\` = プロセスフロー (本文は \`- 段階 | 詳細\` 3〜5 行)
- \`## CARDS タイトル\` = 3 カードグリッド (本文は \`- 見出し | 説明\` 3 行)
